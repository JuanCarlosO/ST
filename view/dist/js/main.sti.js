$(document).ready(function() {
    ...
    if ( url == '?menu=general'  ) {
        ...
        apply_active(url);
        listar_bienes(); 
        autocomplete_input('sp');
        frm_reasignar();
        frm_editar_bien();
    }else if(url == '?menu=reparacion_int'){
        bienes_c_refaccion();
        autocompletado('sp','sp_id',1);
        autocompletado('bien','bien_id',9);
        autocompletado('refaccion','refaccion_id',10);
        frm_asignar_refaccion();
        grupos('grupo','t_bien');
        materiales('material');
        marca('marca');
        modelo('modelo');
...
function grupos(id,select_tb){
    ...
    $('#'+id).change(function(e) {
        e.preventDefault();
        tipo_bienes(select_tb, $(this).val() );
    });
    return false;
}
function tipo_bienes(select,valor) {
    $.ajax({
        url: 'controller/puente.php',
        type: 'POST',
        dataType: 'json',
        data: {option: 8,grupo:valor},
        async: false ,
        cache: false,
    })
    .done(function(data) {
...
//Mostrar listado de bienes con refacciones data: {option: '60'},
function bienes_c_refaccion()
{
    console.log('Cargar bienes con refacciones');
    return false;
}
...
function frm_reasignar() 
{
    $('#frm_reasignar').submit(function(e) {
        e.preventDefault();
        $.ajax({
            url: 'controller/puente.php',
            type: 'POST',
        ...
        .done(function(data) {
            alerta(data.estado,data.message,'modal_reasigna_alert','modal_estado','modal_message');
        })
        .fail(function(jqXHR,errorThrown,textStatus) {
            alerta('error',jqXHR.responseText,'modal_reasigna_alert','modal_estado','modal_message');
        });
...
function listar_bienes() {
    autocompletado('sp','sp_h',1);
    autocompletado('areas','areas_h',3);
    var datos = {
        class: 'table-striped table-bordered',
        columnas: [
            { leyenda: 'ID', class:'text-center', style: 'width:30px;',ordenable:true,columna:'id'},
            { leyenda: 'Tipo',class:'text-center', style: 'width:100px;'},
            { leyenda: 'Estado',class:'text-center', style: 'width:100px;',columna:'status', filtro:function(){
                return anexGrid_select({
                    selected:0,
                    data:[
                        {valor:'',contenido:'Todos'},
                        {valor:'1',contenido:'Bueno'},
                        {valor:'2',contenido:'Regular'},
                        {valor:'3',contenido:'Malo'},
                        {valor:'4',contenido:'Baja'},
                        {valor:'5',contenido:'Malo p/baja'},
                        {valor:'6',contenido:'Acta'}
                    ]
                });
            }},
            { leyenda: 'Grupo',class:'text-center', style: 'width:100px;'},
            { leyenda: 'No. Inventario',class:'text-center', style: 'width:120px;',filtro:true,columna:'inventario'},
            { leyenda: 'Serie', class:'text-center', style: 'width:50px;' ,filtro:true,columna:'serie'},
            { leyenda: 'Marca', class:'text-center', style: 'width:50px;'},
            { leyenda: 'Modelo', class:'text-center', style: 'width:50px;'},
            { leyenda: 'Asignado a', class:'text-center', style: 'width:50px;',columna:'personal_id',filtro:function(){
            ...
            { leyenda: 'Reasignar', class:'text-center', style: 'width:50px;'},
            { leyenda: 'Editar', class:'text-center', style: 'width:50px;'},
        ],
        modelo: [
            ...
            {class:'text-center', formato: function(tr, obj, valor){
                ...
                }else if ( obj.status == 'REGULAR' ){
                    tr.addClass('bg-orange');
                }else if ( obj.status == 'MALO' ){
            ...
            {class:'text-center', formato: function(tr, obj, valor){
                return obj.asignadoa;
            }},
            {class:'text-center', formato: function(tr, obj, valor){
                return anexGrid_boton({
                    type:'button',
                    value:obj.id,
                    class:'btn btn-default btn-flat btn-reasignar',
            ...
            {class:'text-center', formato: function(tr, obj, valor){
                ...
                return anexGrid_boton({
                    type:'button',
                    value:fila_id,
                    class:'btn btn-default btn-flat btn-editar',
    ...
    var tabla = $("#bienes").anexGrid(datos);
    tabla.tabla().on('click', '.btn-reasignar', function(){
        var id = $(this).val();
        $('#modal_reasignar').modal('toggle');
        $('[name="bien_id"]').val(id);
    });
    tabla.tabla().on('click', '.btn-editar', function(){
        ...
        console.log(obj);
        $('#modal-editar-bien').modal('toggle');
        $('[name="bien_id"]').val(obj.id);
        $('#serie').text(obj.serie);
        var grupo = getGrupos(true,obj.grupo,'grupo_edit');
        getTipoBien(true,obj.tipo,grupo,'tipo_edit');
        getMaterial(true,obj.material,'material_edit');
        getStatus(true,obj.status,'estado_edit');
        $('#descripcion_edit').text(obj.descripcion);
        getMarca(true,obj.marca,'marca_edit');
        getModelo(true,obj.modelo,'modelo_edit');
        getColor(true,obj.color,'color_edit');
        $('#inventario_edit').val(obj.inventario);
        $('#serie_edit').val(obj.serie);
        $('#desc_ub_edit').val(obj.desc_ub);
        $('#factura_edit').val(obj.factura);
        $('#importe_edit').val(obj.importe);
        $('#adquisicion_edit').val(obj.fecha_adq);
        getProveedor(true,'','proveedor_edit');
...
function autocompletado(id,hidden,option) 
{
    //Personal 1
    //Areas 3
    // Bienes 9
...
function alerta(estado,mensaje,div,txt_estado,txt_message) {
    var clase = "";
    var estado_msj = "";
    if (estado == 'success' ) {
        clase = "alert-success";
    ...
    $('#'+div).addClass(clase);
    $('#'+txt_estado).text(estado_msj);
    $('#'+txt_message).text(mensaje);
    setTimeout(function(){
        $('#'+div).addClass('hidden');
...
function getGrupos(buscar=false,selected='',contenedor) {
    var valor = '';
    $.ajax({
        url: 'controller/puente.php',
        type: 'POST',
    ...
    .done(function(response) {
        $('#'+contenedor).html('');
        if (selected == '') {
            $('#'+contenedor).append('<option value="" selected>....</option>');
        }else{
            $('#'+contenedor).append('<option value="">....</option>');
        }
        if (buscar) {
...
function getTipoBien(buscar=false,selected='',grupo=1,contenedor) {
    $.ajax({
        url: 'controller/puente.php',
        type: 'POST',
        dataType: 'json',
        data: {option: '8',grupo:grupo},
        async:false,
        cache:false
    })
    .done(function(response) {
...
function getMaterial(buscar=false,selected='',contenedor) {
    $.ajax({
        url: 'controller/puente.php',
        type: 'POST',
        dataType: 'json',
...
function getStatus(buscar=false,selected='',contenedor) {
    var estados = ['','BUENO','REGULAR','MALO','BAJA','MALO PARA BAJA','ACTA'];
    for (var i = 0; i < 7; i++) {
        if (estados[i] == selected) {
            if (selected == ''){
                $('#'+contenedor).append('<option value="" selected>DEBE ASIGNAR ESTATUS</option>');
            }else{
                $('#'+contenedor).append('<option value="'+i+'" selected>'+estados[i]+'</option>');
            }
            
        }else{
            $('#'+contenedor).append('<option value="'+i+'">'+estados[i]+'</option>');
        }
    }
}
function getMarca(buscar=false,selected='',contenedor) {
    $.ajax({
        url: 'controller/puente.php',
        type: 'POST',
        dataType: 'json',
...
function getModelo(buscar=false,selected='',contenedor) {
    $.ajax({
        url: 'controller/puente.php',
        type: 'POST',
        dataType: 'json',
...
function getColor(buscar=false,selected='',contenedor) {
    $.ajax({
        url: 'controller/puente.php',
        type: 'POST',
        dataType: 'json',
...
function getProveedor(buscar=false,selected='',contenedor) {
    $.ajax({
        url: 'controller/puente.php',
        type: 'POST',
        dataType: 'json',
...
function frm_asignar_refaccion() {
    $('#frm-asignar-refaccion').submit(function(e) {
        e.preventDefault();
        ...
        .done(function(response) {
            alerta(response.status,response.message,'alert-asignar','estado-asignar','message-asignar') ;
            document.getElementById('frm-asignar-refaccion').reset();
        })
        .fail(function(jqXHR,errorThrown,textStatus) {
            alerta('error',jqXHR.responseText,'alert-asignar','estado-asignar','message-asignar') ;
        });
        
    });
    return false;
}
function frm_add_refaccion() {
    //alerta(data.estado,data.message,'alerta-editar','editar-estado','editar-mensaje');
    $('#frm-add-refaccion').submit(function(e) {
        e.preventDefault();
        var dataForm = $(this).serialize();
        $.ajax({
            url: 'controller/puente.php',
...
function list_refacciones() {
    var datos = {
        class: 'table-striped table-bordered',
        columnas: [
            { leyenda: 'ID', class:'text-center', style: 'width:30px;',ordenable:true,columna:'id'},
            { leyenda: 'Grupo',class:'text-center', style: 'width:100px;'},
            { leyenda: 'Tipo',class:'text-center', style: 'width:100px;',columna:'tipo_b'},
            { leyenda: 'Material',class:'text-center', style: 'width:100px;'},
            { leyenda: 'Estado',class:'text-center', style: 'width:120px;',filtro:function () {
                return anexGrid_select({
                    selected:0,
                    data:[
                        {valor:'',contenido:'Todos'},
                        {valor:'1',contenido:'Bueno'},
                        {valor:'2',contenido:'Regular'},
                        {valor:'3',contenido:'Malo para reparar'},
                        {valor:'4',contenido:'Malo p/baja'},
                        {valor:'5',contenido:'Baja'},
                        {valor:'6',contenido:'Acta'}
                    ]
                });
            }},
            { leyenda: 'Marca', class:'text-center', style: 'width:50px;' },
            { leyenda: 'Modelo', class:'text-center', style: 'width:50px;' },
            { leyenda: 'Inventario', class:'text-center', style: 'width:50px;',filtro:true,columna:'inventario'},
            { leyenda: 'Serie', class:'text-center', style: 'width:50px;',filtro:true,columna:'serie'},
            { leyenda: 'Descripción', class:'text-center', style: 'width:50px;'},
            { leyenda: 'Asignaciones', class:'text-center', style: 'width:50px;',columna:'asignacion',filtro:function(){
                return anexGrid_select({
                    selected:0,
                    data:[
                        {valor:'',contenido:'Todos'},
                        {valor:'1',contenido:'Sin asignar'},
                        {valor:'2',contenido:'Asignados'},
                    ]
                });
            }},
        ],
        modelo: [
            ...
            { propiedad: 'grupo', class:'text-center' },
            { propiedad: 'tipo_b', class:'text-center' },
            { propiedad: 'material', class:'text-center' },
            {class:'text-center', formato: function(tr, obj, valor){
                if ( obj.estado == 'BUENO' ) {
                    tr.addClass('bg-green-active');
                }else if ( obj.estado == 'REGULAR' ){
                    tr.addClass('bg-orange');
                }else {
                    tr.addClass('bg-red-active');
                }
            ...
            { propiedad: 'marca', class:'text-center' },
            { propiedad: 'modelo', class:'text-center' },
            { propiedad: 'inventario', class:'text-center' },
            { propiedad: 'serie', class:'text-center' },
            { propiedad: 'descripcion', class:'text-center' },
            { class:'text-center', formato: function(tr, obj, valor){
                
                if (obj.asignacion == 'ASIGNADO') {
                    return '<button class="btn btn-default btn-flat btn-detalle" onclick="detalle_asignacion('+obj.id+');" data-toggle="tooltip" title="Detalle de la asignacion" value="'+obj.id+'"><i class="fa fa-eye"></i></button>';
                }else{
                    return '<label>SIN ASIGNAR</label>'
                }
            }},
        ],
    ...
    var tabla = $("#tbl_refacciones").anexGrid(datos);
    
    return tabla;
}
function detalle_asignacion(refaccion) {
    
    $.ajax({
        url: 'controller/puente.php',
        type: 'POST',
        dataType: 'json',
        data: {option: '87',ref:refaccion},
        async:false,
    })
    .done(function(response) {
        $('#modal_detalle_asignacion_refaccion').modal('toggle');
        var equipo = "Marca: "+response.marca+" Tipo: "+response.tipo+" "+response.claves;
        $('#eq_asignado').text(equipo);
        $('#ref_asignada').text(response.refac);
        $('#f_asignacion').text(response.f_asigna);
        $('#us_asignado').text(response.asignado);
    })
    .fail(function() {
        alert("No se puedo obtener el detalle de la asignación de este bien.");
    });
...
function get_tipo_doc(input) {
    ...
    .done(function(response) {
        ...
        $.each(response, function(i, val) {
            $('#'+input).append('<option value="'+val.id+'">'+val.nombre+'</option>');
        });
    })
