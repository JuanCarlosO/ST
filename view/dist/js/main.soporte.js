...
function getURL() {
	...
	if ( URLsearch == '?menu=repair' ) {
		listar_reparaciones();		
	}else if ( URLsearch == '?menu=general' ){
		listar_solicitudes();
	}
}
function listar_reparaciones(contenedor) {
	var datos = {
		class: 'table-striped table-bordered',
		columnas: [
			{ leyenda: 'ID', style: 'width:200px;', columna: 'Profesion_id' },
			{ leyenda: 'Caracteristicas del equipo', columna: 'Nombre', filtro: true },
			{ leyenda: 'Reparaciones realizadas', style: 'width:300px;', columna: 'Correo' },
			{ leyenda: 'Reparado', style: 'width:120px;' },
			{ leyenda: 'Baja', style: 'width:100px;', columna: 'Sueldo' },
		],
		modelo: [
	...
	$("#reparaciones_list").anexGrid(datos);
}
function listar_solicitudes() {
	var datos = {
		class: 'table-striped table-bordered',
		columnas: [
			{ leyenda: 'ID', style: 'width:30px;',ordenable:true,columna:'id'},
			{ leyenda: 'Detalle de la solicitud', style: 'width:300px;'},
			{ leyenda: 'Detalle de la atención', style: 'width:300px;'},
			{ leyenda: 'Estado', style: 'width:120px;',columna:'estatus',filtro:function(){
				return anexGrid_select({
					data:[
							{ valor: '', contenido: 'Todos'},
							{ valor: 1, contenido: 'Sin atender'},
							{ valor: 2, contenido: 'Visto'},
							{ valor: 3, contenido: 'Atendido'},
							{ valor: 4, contenido: 'Evaluado'},
							{ valor: 5, contenido: 'Cancelado'},
							{ valor: 6, contenido: 'En raparación'},
							{ valor: 7, contenido: 'Reparado'}
						]
				});
			}},
			{ leyenda: 'Visto', class:'text-center', style: 'width:50px;'},
			{ leyenda: 'Atender', class:'text-center', style: 'width:50px;'},
			{ leyenda: 'Cancelar', class:'text-center', style: 'width:50px;'},
		],
		modelo: [
			{ formato: function(tr, obj, valor){
				return obj.id;
			...
			{ formato: function(tr, obj, valor){
				return 	'<ul>'+
							'<li><b>Falla: </b>'+obj.falla+'</li>'+
							'<li><b>Fecha de solicitud: </b>'+obj.fecha+'</li>'+
						'</ul>';
			}},
		
			{ formato: function(tr, obj, valor){
				return 	'<ul>'+
							'<li><b>Solución: </b>'+obj.solucion+'</li>'+
							'<li><b>Fecha de solución: </b>'+obj.f_solucion+'</li>'+
						'</ul>';
			}},
			{class:'text-center', formato: function(tr, obj, valor){
				if ( obj.estatus == 'Atendido' ) {
					tr.addClass('bg-green-active');
				}else if ( obj.estatus == 'Evaluado' ){
					tr.addClass('bg-teal-active');
				}else if ( obj.estatus == 'Sin atender' ){
					tr.addClass('bg-orange');
				}else if ( obj.estatus == 'Visto' ){
					tr.addClass('bg-purple');
				}else if ( obj.estatus == 'Cancelado' ){
					tr.addClass('bg-red-active');
				}
				return obj.estatus;
			}},
			{class: 'text-center', formato: function(tr, obj, valor){
				if (obj.estatus == 'Sin atender') {
					return anexGrid_boton({
						type:'button',
						value: obj.reparacion,
						contenido:'<i class="fa fa-eye"></i>',
						class:'btn btn-default btn-flat ',
						attr:[
							'onclick = "ViewSol('+obj.reparacion+');"'
						]
					});
				}
			}},
			{ class: 'text-center', formato: function(tr, obj, valor){
				if (obj.estatus == 'Sin atender' || obj.estatus == 'Visto') {
					return anexGrid_boton({
						type:'button',
						value: '',
						...
						attr:[
							...
							'onclick = "load_devices('+obj.reparacion+','+obj.afectado_id+');"'
						]
					});
				}
			}},
			{ class: 'text-center', formato: function(tr, obj, valor){
				if ( obj.estatus == 'Sin atender' || obj.estatus == 'Visto') {
					return anexGrid_boton({
						type:'button',
						value: '',
						contenido:'<i class="fa fa-times text-red"></i>',
						class:'btn btn-default btn-flat ',
						attr:[
							'onclick = "CancelarSol('+obj.reparacion+');"'
						]
	...
	var tabla = $("#solicitudes_list").anexGrid(datos);
	return tabla
}
/**/
function CancelarSol(id) {
	$.ajax({
		url: 'controller/puente.php',
		type: 'POST',
		dataType: 'json',
	...
	.done(function(data) {
		if ( data.message =='success' ) {
			...
			$('#success').removeClass('hidden');
			/*Colocar el contenido*/
			$('#mensaje_exitoso').text('La solicitud a sido cancelada.');
			/*Moverse hasta el mensaje de exito*/
			window.location.href = '#success';
			/*Desaparecer el mensaje*/
			setTimeout(function(){
			...
			$('#error').removeClass('hidden');
			/*Colocar el contenido*/
			$('#mensaje_erroneo').text('No se pudo cancelar la solicitud');
			/*Moverse hasta el mensaje de exito*/
			window.location.href = '#error';
			/*Desaparecer el mensaje*/
			setTimeout(function(){
